...
function render() { renderAssets(); renderEngines(); renderFormats(); renderPreview(); renderEvents(); }
$("#target").addEventListener("change", (event) => { state.target = event.target.value; $("#status").textContent = `Routing future patches through the ${state.target} adapter.`; renderEngines(); });
